...
export default {
  ...
  methods: {
    async createNewCassava() {
      ...
      var newpindata = [];
      await axios
        .get("http://188.166.246.244:6969/area-check/")
        .then((response) => {
    ...
    async starteditCassava() {
      ...
      var i = 0;
      var newpindata = [];
      await axios.get("http://188.166.246.244:6969/area-check/").then((response) => {
        while (i < response.data.length) {
      ...
      var newxdata = [];
     await axios
        .get("http://188.166.246.244:6969/cassava-check/")
        .then((response) => {
          while (i < response.data.length) {
            if (
   ...
   async saveEditCassava() {
      ...
      var p = 0;
     await axios
        .put("http://188.166.246.244:6969/area-check/" + this.selectedarea + "/", {
      ...
      while (i < this.xdata.length) {
        ...
        while (g < this.alreadyhaveid.length) {
          if (this.xdata[i].id == this.alreadyhaveid[g]) {
           await axios
              .put(
                "http://188.166.246.244:6969/cassava-check/" + this.xdata[i].id + "/",
                {
        ...
        if (p == 0) {
         await axios
            .post("http://188.166.246.244:6969/cassava-check/", {
              cassava_area: this.selectedarea,
              check_date: this.date,
              latitude: this.latposition,
              longtitude: this.lngposition,
      ...
      while (m < this.deletecassavaid.length) {
        axios.delete(
          "http://188.166.246.244:6969/cassava-check/" + this.deletecassavaid[m] + "/",
          {}
        );
        m++;
      }
   ...
   async addNewArea() {
     ...
     await axios
        .post("http://188.166.246.244:6969/area-check/", {
          farm_store: this.idfarm,
          starch_percentage: 45,
          tree_latitude: this.latposition,
          tree_longtitude: this.lngposition,
    ...
    deleteArea() {
      ...
      axios.delete(
          "http://188.166.246.244:6969/area-check/" + this.selectedarea + "/",
          {}
        );
      const vm = this;
      vm.$router.push("/show-area");
